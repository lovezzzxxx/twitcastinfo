...
while true; do
	...
	curl -s "https://twitcasting.tv/${PART_URL}/${PAGE_LINK}" | awk 'BEGIN{RS="<a class=\"tw-movie-thumbnail\"|<div class=\"paging\"";FS="[\n<>]";ORS="\n";OFS="\t"} $1 ~ /href/ {if (NF<100) {split($1,LINK,"\"");TITLE=$18;gsub("^ *| *$","",TITLE);DATE=$30;print "NO_REC",LINK[2],DATE,TITLE} ; if (NF>100) {split($1,LINK,"\"");TITLE=$30;gsub("^ *| *$","",TITLE);DATE=$42;print "RECORD",LINK[2],DATE,TITLE}}'
